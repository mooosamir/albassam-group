    ...
    def action_confirm(self):
        ...
        for production in self:
            if production.move_byproduct_ids:
                move_byproduct_qty = []
                [move_byproduct_qty.append(by_product.product_uom_qty) for by_product in production.move_byproduct_ids]
            ...
            if production.move_raw_ids:
                move_raw_qty = []
                [move_raw_qty.append(raw_product.product_uom_qty) for raw_product in production.move_raw_ids]
                if production.product_qty != sum(move_raw_qty):
                    raise ValidationError(_('Total Production Quantity and sum of To Consume Quantity should be same.'))
