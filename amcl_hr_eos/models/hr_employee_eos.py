...
class HrEmployeeEos(models.Model):
    ...
    employee_id = fields.Many2one('hr.employee', "Employee", required=True, readonly=True,
                                  states={'draft': [('readonly', False)], 'confirm': [('readonly', False)]})
    date_of_join = fields.Date(related='employee_id.joining_date', type="date", string="Joining Date", store=True,
                               readonly=True)
    ...
    duration_years = fields.Integer('No. of Years', readonly=True,
                                    states={'draft': [('readonly', False)], 'confirm': [('readonly', False)]})
    type = fields.Selection([('resignation', 'Resignation'), ('termination', 'Termination'), ('death', 'Death')],
                            'Type', readonly=True,
                            states={'draft': [('readonly', False)], 'confirm': [('readonly', False)]})
    ...
    # account
    journal_id = fields.Many2one('account.journal', 'Force Journal', required=1,
    ...
    def _track_subtype(self, init_values):
        ...
        if 'state' in init_values and self.state == 'draft':
            return 'amcl_hr_eos.mt_employee_eos_new'
        elif 'state' in init_values and self.state == 'confirm':
            return 'amcl_hr_eos.mt_employee_eos_confirm'
        elif 'state' in init_values and self.state == 'accepted':
            return 'amcl_hr_eos.mt_employee_eos_accept'
        elif 'state' in init_values and self.state == 'validate':
            return 'amcl_hr_eos.mt_employee_eos_validate'
        elif 'state' in init_values and self.state == 'done':
            return 'amcl_hr_eos.mt_employee_eos_done'
        elif 'state' in init_values and self.state == 'cancelled':
            return 'amcl_hr_eos.mt_employee_eos_cancel'
    ...
    def calc_eos(self):
        ...
        for eos in self:
            join_date = datetime.strptime(str(eos.date_of_join), DEFAULT_SERVER_DATE_FORMAT)
            if not eos.date_of_leave:
                raise UserError(_('Please define employee date of leaving!'))
            leave_date = datetime.strptime(str(eos.date_of_leave), DEFAULT_SERVER_DATE_FORMAT)
            ...
            eos.write(
                {'duration_days': duration_days, 'duration_months': duration_months, 'duration_years': duration_years})
            selected_date = datetime.strptime(str(eos.date_of_leave), DEFAULT_SERVER_DATE_FORMAT)
            ...
            wages = self.env['hr.contract'].browse(contract_ids[0]).total_salary
            total_eos = 0.0
            if duration_years >= 2 and duration_years < 5:
                _logger.critical('rewertetewtwetwetwetwetwe')
                _logger.critical(duration_years)
                total_eos = ((wages / 2) * duration_years) + (((wages / 2) / 12) * duration_months) + (
                        (((wages / 2) / 12) / 30) * duration_days)
                _logger.critical(total_eos)
            elif duration_years >= 5 and duration_years < 10:
                total_eos = ((wages / 2) * duration_years) + ((wages / 12) * duration_months) + (
                ...
                raise UserError('Duration Less 2 Years')
            # elif duration_years < 2:
            #     raise UserError('duration_years' + str(duration_years) + '\nduration')
            # raise UserError(duration_years)
            # _logger.critical('==T==O==T==A==L==')
            # _logger.critical(total_eos)
            ...
            if days < 1825:
                total_eos = (days * (wages / 2) / 365)
            else:
                total_eos = (1825 * (wages / 2) / 365) + (
                        ((days - 1825) * wages) / 365)
            ...
            # onchange_result = eos.payslip_id.onchange_employee_id(date_from, date_to, eos.employee_id.id,
            #                                                       contract_ids[0])
            # vals1 = onchange_result['value']
            # if vals1.get('worked_days_line_ids'):
            #     vals1['worked_days_line_ids'] = [[5, False, False]] + [[0, False, line] for line in
            #                                                            vals1['worked_days_line_ids']]
            # if eos.payslip_id.input_line_ids:
            #     eos.payslip_id['input_line_ids'] = [[5, False, False]] + [[0, False, line] for line in
            #                                                               vals1['input_line_ids']]
            ...
            eos.write({
                'current_month_salary': net, })  # 'basic_salary': basic_salary, 'hra': hra, 'ta': ta, 'other_allowance': other
            ...
            if eos.type == 'resignation':
                if eos.calc_year > 2 and eos.calc_year < 5:
                    ...
                    payable_eos = total_eos
            _logger.critical(payable_eos)
            # Annual Leave Calc
            # holiday_status_pool = self.env['hr.holidays.status']
            holiday_status_pool = self.env['hr.leave.type']
            holiday_status_ids = holiday_status_pool.search([('carry_forward', '=', True)])
            annual_leave_amount = 0.0
            remaining_leaves = 0.0

            annual_leave = self.env['hr.leave.type'].search([('annual', '=', True)], limit=1)
            leave_allocation = self.env['hr.leave.allocation'].search(
                [('employee_id', '=', self.employee_id.id),
                 ('holiday_status_id', '=', annual_leave.id)])
            params = []
            query = """
                SELECT sum(number_of_days) FROM hr_leave 
                WHERE employee_id=%s AND holiday_allocation_id=%s; 
            """
            params.append(self.employee_id.id)
            params.append(leave_allocation.id)
            self.env.cr.execute(query, params)
            data = self.env.cr.dictfetchone()
            remaining_leaves = leave_allocation.number_of_days - data['sum']
            annual_leave_amount = (wages / 30) * remaining_leaves
            eos.write({'total_eos': payable_eos, 'remaining_leave': remaining_leaves,
            ...
            #     remaining_leaves = (allocate_leave_month - leaves_taken) + leave_values[holiday_status_ids[0].id][
    ...
    def onchange_employee_id(self):
        ...
        if self.employee_id:
            ...
            selected_date = datetime.strptime(str(self.employee_id.date_of_leave), DEFAULT_SERVER_DATE_FORMAT)
            ...
            if not contract_ids:
                raise UserError(_('Please define contract for selected Employee!'))
            join_date = datetime.strptime(str(self.employee_id.joining_date), DEFAULT_SERVER_DATE_FORMAT)
            leave_date = datetime.strptime(str(self.employee_id.date_of_leave), DEFAULT_SERVER_DATE_FORMAT)
    ...
    def eos_validate(self):
        ...
        for record in self:
            finance_groups_config_ids = finance_groups_config_obj.search(
                [('branch_id', '=', record.employee_id.branch_id.id or False), ('finance_ids', '!=', False)])
    ...
    def account_move_get(self):
        ...
        if self.journal_id:
            ...
            if not journal_id:
                raise UserError(
                    _("No EOS journal found. Please make sure you have a journal with type 'purchase' configured."))
    ...
    def action_receipt_create(self):
        ...
        for eos in self:
            ...
            acc = self.account_id
            print('Accounts :: %s ', acc)
            if not acc:
                raise UserError(_('Please select the EOS Account'))
            ...
            lines = map(lambda x: (0, 0, self.line_get_convert(x, eos.employee_id.address_home_id, eos.date_confirm)),
                        eml)
...
class PayslipClass(models.Model):
    _inherit = 'hr.payslip'
